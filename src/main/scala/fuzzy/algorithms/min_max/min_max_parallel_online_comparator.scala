package fuzzy.algorithms.min_max
...
class MinMaxParallelOnlineComparator(
  ...
  when(io.start === true.B) {
    ...
    switch(state) {
      ...
      is(sStarted) {
        ...
        when(
          currentIteration =/= vectorCount.U - 1.U + /* initiation delay */ log2Ceil(
            vectorCount
          ).U
        ) {
            ...
            if (i >= 0 && i <= (math.pow(2, (vectorCount / 2) - 1).toInt - 2)) {
              ...
              val (selectedInput, earlyTerminate1, earlyTerminate2, maxOutput) =
                OnlineComparator2(debug, true)(
                  ...
                  if (i % 2 != 0)
                    regStorageVec(math.ceil((i - 1) / 2).toInt).earlyTerminated1
                  else
                    regStorageVec(math.ceil((i - 1) / 2).toInt).earlyTerminated2
                )
              ...
              if (debug) {
                ...
                printf(
                  "\tin1 : regStorageVec(%d).minMaxOutput = %d\n",
                  (2 * i + 1).U,
                  regStorageVec(2 * i + 1).minMaxOutput
                );
                printf(
                  "\tin2 : regStorageVec(%d).minMaxOutput = %d\n",
                  (2 * i + 2).U,
                  regStorageVec(2 * i + 2).minMaxOutput
                );
                if (i % 2 != 0) {
                  printf(
                    "\tET : regStorageVec(%d).earlyTerminated1\n",
                    (math.ceil((i - 1) / 2).toInt).U
                  );
                } else {
                  printf(
                    "\tET : regStorageVec(%d).earlyTerminated2\n",
                    (math.ceil((i - 1) / 2).toInt).U
                  );
                }
              }
