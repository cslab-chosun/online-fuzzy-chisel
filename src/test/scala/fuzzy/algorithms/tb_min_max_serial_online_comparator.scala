		...
		"DUT" should "pass" in {
			...
			test(new MinMaxSerialOnlineComparator(DesignConsts.ENABLE_DEBUG, DesignConsts.VECTOR_COUNT, DesignConsts.NUMBER_LENGTH)) { dut =>
				...
				for (i <- 0 until DesignConsts.VECTOR_COUNT) {
					...
					println(s"inpdddut 1 : 0x${dut.io.in1.peek().litValue.toInt.toHexString} (${dut.io.in1.peek().litValue.toInt.toBinaryString}), input 2 : 0x${dut.io.in2.peek().litValue.toInt.toHexString} (${dut.io.in2.peek().litValue.toInt.toBinaryString})\n")
