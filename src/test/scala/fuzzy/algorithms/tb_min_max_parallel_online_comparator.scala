...
class MinMaxParallelOnlineComparatorTest extends AnyFlatSpec with
	ChiselScalatestTester {
		"DUT" should "pass" in {
			...
			test(new MinMaxParallelOnlineComparator(DesignConsts.ENABLE_DEBUG, DesignConsts.VECTOR_COUNT)) { dut =>
  				...
  				for (i <- 0 until DesignConsts.NUMBER_LENGTH) {
					...
					println(s"\ninput 1 : \n")
  					...
  					for (j <- 0 until DesignConsts.VECTOR_COUNT) {
	    				...
	    				dut.io.in1(j).poke(TestingSample.input1_bytes(j)(DesignConsts.NUMBER_LENGTH - i - 1))
					...
					println(s"\ninput 2 : \n")
  					...
  					for (j <- 0 until DesignConsts.VECTOR_COUNT) {
	    				...
	    				dut.io.in2(j).poke(TestingSample.input2_bytes(j)(DesignConsts.NUMBER_LENGTH - i - 1))
